-- =====================================================
-- FIXED SUPABASE MIGRATION (Handles Existing Data)
-- Run this script to complete your database setup
-- =====================================================

-- MIGRATION 001: Initial Schema (Skip if exists)
-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pg_trgm";

-- Create user profiles table (only if not exists)
...
-- Add trigger to profiles table (only if not exists)
DO $$
BEGIN
...
-- Create indexes for performance (only if not exist)
...
-- MIGRATION 002: Prompt Templates (Skip if exists)
...
-- Create full-text search index (only if not exists)
CREATE INDEX IF NOT EXISTS idx_prompt_templates_fts ON prompt_templates 
  USING gin(to_tsvector('english', title || ' ' || COALESCE(description, '') || ' ' || content));

-- Create other indexes (only if not exist)
...
CREATE INDEX IF NOT EXISTS idx_prompt_templates_rating_average ON prompt_templates(rating_average);
...
-- MIGRATION 003: AI Usage Quotas (Skip if exists)
...
-- Add triggers (only if not exist)
DO $$
BEGIN
...
-- Insert default AI provider configurations (only if not exist)
...
-- MIGRATION 004: Subscription Management (Skip if exists)
...
CREATE INDEX IF NOT EXISTS idx_feature_entitlements_feature_name ON feature_entitlements(feature_name);
CREATE INDEX IF NOT EXISTS idx_feature_entitlements_is_enabled ON feature_entitlements(is_enabled);
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_feature_entitlements_updated_at') THEN
...
-- Insert default subscription plans (only if not exist)
...
-- Insert default feature entitlements for free users (only if not exist)
